...
class PillTableViewController: UITableViewController {
    ...
    var pill: Prescription?
    
    override func viewDidLoad() {
        super.viewDidLoad()
        pillImage.layer.cornerRadius = pillImage.bounds.size.height/2
        pillImage.layer.borderWidth = 1.0
        pillImage.clipsToBounds = true
        pillImage.image = pill?.image
        nameLabel.text = pill?.name
        timeLabel.text = pill?.time
        dosageLabel.text = pill?.dosage.description
        desLabel.text = pill?.des
